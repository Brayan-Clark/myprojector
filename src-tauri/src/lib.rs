...
fn fetch_hymns(_app_handle: tauri::AppHandle, db_name: &str) -> Result<Vec<Song>, String> {
    ...
    let mut stmt = conn.prepare("SELECT id, c_num, c_title, c_content FROM adventiste_cantique").map_err(|e| e.to_string())?;
...
#[tauri::command]
fn update_song(db_name: &str, is_bible: bool, id: i64, title: String, content: String) -> Result<(), String> {
    let mut db_path = std::env::current_dir().map_err(|e| e.to_string())?;
    db_path.push("data");
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![fetch_hymns, fetch_bible, update_song, list_dbs, download_db])
